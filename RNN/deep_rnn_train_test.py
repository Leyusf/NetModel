...
num_hiddens = 256
# deep_rnn_layer = RNNLayer(len(vocab), num_hiddens, num_layers=3)
deep_rnn_layer = nn.RNN(len(vocab), num_hiddens, num_layers=3)

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
net = RNNModel(deep_rnn_layer, vocab_size=len(vocab))
net = net.to(device)
